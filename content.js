(() => {
  ...
  let lastCheckedText  = '';
  let lastCheckedResult = null; // { corrected, categories }

  const MAX_WORDS = 500;
  const MIN_CHECK_INTERVAL = 8000; // ms — minimum gap between API calls

  let settings = { spelling:true, grammar:true, clarity:true, tone:true, delay:2000, enabled:true, pausedSites:[], autoCheck:true };
  ...
  function refreshProviderChip() {
    try {
      chrome.storage.sync.get(['provider'], (d) => {
        ...
        if (tag) {
          tag.textContent = p === 'claude' ? 'Claude' : 'Gemini';
          tag.className = '__ww-chip ' + p;
        }
      });
    } catch(e) {}
  }
  ...
  async function triggerCorrection(el, text) {
    ...
    // Cache hit — replay last result without an API call
    if (text === lastCheckedText && lastCheckedResult !== null) {
      const { corrected, categories } = lastCheckedResult;
      if (corrected === text) {
        changeCount = 0; correctedText = '';
        setBadge('idle'); showAnchor(el, 'done'); showBubble(el, 'ok');
      } else {
        originalText = text; correctedText = corrected;
        const { changes } = buildDiff(originalText, correctedText);
        changeCount = changes;
        showBubble(el, 'suggestion', null, categories);
        setBadge('done'); showAnchor(el, 'done');
      }
      return;
    }

    // Cooldown — reschedule if last call was too recent
    const elapsed = Date.now() - lastTriggerTime;
    if (lastTriggerTime > 0 && elapsed < MIN_CHECK_INTERVAL) {
      debounceTimer = setTimeout(() => triggerCorrection(el, text), MIN_CHECK_INTERVAL - elapsed);
      return;
    }
    ...
    try {
      ...
      if (!response.success) {
        ...
        if (err === 'NO_API_KEY' || isInvalidKey) {
          ...
          const detail = parts.length > 2 ? `${parts[1]}: ${parts[2]}` : null;
          showBubble(el, 'rate-limit', el, null, detail);
          showAnchor(el, 'unchecked');
        } else {
      ...
      lastCheckedResult = { corrected, categories };
  ...
  function showBubble(target, state, errorMsgOrEl, categories, rateLimitDetail) {
  ...
  <div class="__ww-skel-lines">
    ...
    <div class="__ww-skel-line" style="width:58%"></div>
      ...
      const detailLine = rateLimitDetail ? `<div class="__ww-nokey-sub" style="margin-top:6px;color:#e2673a;font-size:11px">${escHtml(rateLimitDetail)}</div>` : '';
      bubble.innerHTML = `
        <div class="__ww-nokey">
      ...
      bubble.innerHTML = `
        <div class="__ww-row __ww-ok">
          <div class="__ww-ok-icon">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
              <circle cx="8" cy="8" r="8" fill="#5cb87a"/>
              <path d="M4 8L7 11L12 5" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </div>
          <span>Text looks great — no changes needed.</span>
        </div>`;
    }

    bubble.style.display = 'block';
    requestAnimationFrame(() => positionBubble(target));
    if (state === 'ok') setTimeout(() => hideBubble(), 2500);
  }
