...
document.getElementById('ob-continue').addEventListener('click', () => {
  ...
  // Update key link and placeholder based on provider
...
function loadMainAppData() {
  const storageKeys = ['provider','claudeKey','geminiKey','spelling','grammar','clarity','tone','enabled',
...
function switchProvider(provider, save = true) {
  ...
  document.getElementById('tab-gemini').className = 'provider-tab' + (provider === 'gemini' ? ' active-gemini' : '');
  document.getElementById('tab-claude').className = 'provider-tab' + (provider === 'claude' ? ' active-claude' : '');

  const badge = document.getElementById('providerBadge');
  badge.className = 'provider-chip ' + provider;
  const lbl = document.getElementById('providerBadgeLabel');
  if (lbl) lbl.textContent = provider === 'claude' ? 'Claude Haiku' : 'Gemini 2.5 Flash-Lite';
